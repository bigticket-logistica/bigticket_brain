...
function TercerosMX() {
  ...
  const cargarExcel = async (file) => {
    ...
    try {
      ...
      // Normaliza EMPRESA TRANSPORTE: colapsa espacios multiples y bordes.
      // Evita duplicados tipo " JUAN PEREZ" vs "JUAN PEREZ" que generan prefacturas separadas.
      ...
      // ── Deteccion de nombres de empresa duplicados o distintos al historico ──
      // Clave canonica: mayusculas, sin acentos, espacios colapsados. Dos variantes con la
      // misma clave son la misma empresa escrita distinto (case/acentos); se unifican en el panel.
      ...
      // Grafia historica ya cargada en semanas anteriores (sugiere mantener la existente)
      const histPorClave = {};
      ...
      if (grupos.length > 0) {
        ...
        setMsg({ ok: false, txt: "\u26a0 " + grupos.length + " empresa(s) con nombres duplicados o distintos al hist\u00f3rico. Unif\u00edcalos en el panel amarillo y presiona Aplicar y cargar." });
        setCargando(false);
  ...
  return (
    <div style={{ padding: 24 }}>
      ...
      {/* Panel de unificación de nombres de empresa duplicados (previo a la carga) */}
      {dupsPanel && (
        <div style={{ background: "#fffbeb", border: "1.5px solid #fcd34d", borderRadius: 8, padding: "14px 16px", marginBottom: 14 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: "#92400e" }}>⚠ Nombres de empresa con variantes — unifícalos antes de cargar</div>
          <div style={{ fontSize: 11, color: "#92400e", marginTop: 2 }}>
            Variantes por mayúsculas/acentos dentro del Excel, o distintas a la grafía histórica, generan prefacturas separadas para la misma empresa.
